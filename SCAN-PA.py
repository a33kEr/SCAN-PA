...
# -*- coding: utf-8 -*-
# 
# _____  _____   ___   _   _         ______   ___  
#/  ___|/  __ \ / _ \ | \ | |        | ___ \ / _ \ 
#\ `--. | /  \// /_\ \|  \| | ______ | |_/ // /_\ \
# `--. \| |    |  _  || . ` ||______||  __/ |  _  |
#/\__/ /| \__/\| | | || |\  |        | |    | | | |
...
# Tool       : SCAN-PA 
# Usage      : python3 SCAN-PA.py example.com
...
# SCAN-PA Help Context
def helper():
        print(bcolors.OKBLUE+"Information:"+bcolors.ENDC)
        print("------------")
        print("\t./SCAN-PA.py example.com: Scans the domain example.com.")
        print("\t./SCAN-PA.py example.com --skip dmitry --skip theHarvester: Skip the 'dmitry' and 'theHarvester' tests.")
        print("\t./SCAN-PA.py example.com --nospinner: Disable the idle loader/spinner.")
        print("\t./SCAN-PA.py --update   : Updates the scanner to the latest version.")
        print("\t./SCAN-PA.py --help     : Displays this help context.")
        print(bcolors.OKBLUE+"Interactive:"+bcolors.ENDC)
        print("------------")
        print("\tCtrl+C: Skips current test.")
        print("\tCtrl+Z: Quits SCAN-PA.")
...
def logo():
    ...
    logo_ascii = """
                                """+bcolors.BADFAIL+" ADITYA PATHAK (RA2111003010299) _ Presents "+bcolors.WARNING+"""
...
class Spinner:
    ...
    def spinner_task(self):
        ...
        try:
            ...
            print("\n\t"+ bcolors.BG_ERR_TXT+"SCAN-PA received a series of Ctrl+C hits. Quitting..." +bcolors.ENDC)
            sys.exit(1)
...
tool_cmd   = [
                ...
                #2
                ["wget -O /tmp/SCAN-PA_temp_aspnet_config_err --tries=1 ","/%7C~.aspx"],

                #3
                ["wget -O /tmp/SCAN-PA_temp_wp_check --tries=1 ","/wp-admin"],

                #4
                ["wget -O /tmp/SCAN-PA_temp_drp_check --tries=1 ","/user"],

                #5
                ["wget -O /tmp/SCAN-PA_temp_joom_check --tries=1 ","/administrator"],
                ...
                #75
                ["wget -O /tmp/SCAN-PA_temp_aspnet_elmah_axd --tries=1 ","/elmah.axd"],
                ...
                #78
                ["wapiti "," -f txt -o SCAN-PA_temp_wapiti"],
...
if args_namespace.help or (not args_namespace.update \
    ...
    logo()
    print("SCAN-PA is updating....Please wait.\n")
    spinner.start()
    ...
    if rs_internet_availability == 0:
        ...
        spinner.stop()
        sys.exit(1)
    cmd = 'sha1sum SCAN-PA.py | grep .... | cut -c 1-40'
    oldversion_hash = subprocess.check_output(cmd, shell=True)
    oldversion_hash = oldversion_hash.strip()
    os.system('wget -N https://raw.githubusercontent.com/skavngr/rapidscan/master/rapidscan.py -O SCAN-PA.py > /dev/null 2>&1')
    ...
    if oldversion_hash == newversion_hash :
        clear()
        print("\t"+ bcolors.OKBLUE +"You already have the latest version of SCAN-PA." + bcolors.ENDC)
    else:
        clear()
        print("\t"+ bcolors.OKGREEN +"SCAN-PA successfully updated to the latest version." +bcolors.ENDC)
    spinner.stop()
    sys.exit(1)
    ...
    os.system('rm /tmp/SCAN-PA* > /dev/null 2>&1') # Clearing previous scan files
    os.system('clear')
    ...
    while (rs_avail_tools < len(tools_precheck)):
        ...
        try:
            ...
            print("\t"+bcolors.BG_ERR_TXT+"SCAN-PA was terminated abruptly..."+bcolors.ENDC)
            sys.exit(1)
    ...
    if len(unavail_tools_names) == 0:
        print("\t"+bcolors.OKGREEN+"All Scanning Tools are available. Complete vulnerability checks will be performed by SCAN-PA."+bcolors.ENDC)
    else:
        print("\t"+bcolors.WARNING+"Some of these tools "+bcolors.BADFAIL+str(unavail_tools_names)+bcolors.ENDC+bcolors.WARNING+" are unavailable or will be skipped. SCAN-PA will still perform the rest of the tests. Install these tools to fully utilize the functionality of SCAN-PA."+bcolors.ENDC)
    ...
    while(tool < len(tool_names)):
        ...
        temp_file = "/tmp/SCAN-PA_temp_"+tool_names[tool][arg1]
        ...
        if runTest == 1:
                ...
                print("\n"+bcolors.WARNING + "\tTest Skipped. Performing Next. Press Ctrl+Z to Quit SCAN-PA.\n" + bcolors.ENDC)
                rs_skipped_checks = rs_skipped_checks + 1
    ...
    if len(rs_vul_list)==0:
        ...
        with open(vulreport, "a") as report:
            while(rs_vul < len(rs_vul_list)):
                ...
                report.write("\n------------------------\n\n")
                temp_report_name = "/tmp/SCAN-PA_temp_"+vuln_info[arg1]
            ...
            print("\tComplete Vulnerability Report for "+bcolors.OKBLUE+target+bcolors.ENDC+" named "+bcolors.OKGREEN+vulreport+bcolors.ENDC+" is available under the same directory SCAN-PA resides.")
    ...
    for file_index, file_name in enumerate(tool_names):
        with open(debuglog, "a") as report:
            try:
                with open("/tmp/SCAN-PA_temp_"+file_name[arg1], 'r') as temp_report:
                        data = temp_report.read()
    ...
    os.system('rm /tmp/SCAN-PA_te* > /dev/null 2>&1') # Clearing previous scan files
